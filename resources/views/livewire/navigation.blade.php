<header class="bg-trueGray-700 sticky z-50 top-0" x-data="dropdown()">
    <div class="container flex items-center h-16 justify-between md:justify-start">
        <a :class="{'bg-opaciti-100 text-orange-500': open}"
            x-on:click="show()"
        ...
        <a href="/" class="mx-6">
            <x-jet-application-mark class="d-block h-9 w-auto" />
        </a>
    ...
    <nav :class="{'block':open, 'hidden':!open}" id="navigation-menu" class="bg-trueGray-700 bg-opacity-25 absolute w-full hidden">
        {{-- menu escritorio --}}
        <div class="container h-full hidden md:block">
           <div
                x-on:click.await="close()"
                class="grid grid-cols-4 h-full relative">
                <ul class="bg-white">
                    @foreach ($categories as $category)
                        <li class="navigation-link text-trueGray-500  hover:bg-orange-500 hover:text-white">
                            <a href="" class="py-2 px-4 text-sm flex items-center">
                                <span class="d-flex justify-center w-9">{!!$category->icon!!}</span>
                                {{ $category->name }}
                            </a>
        ...
        <div class="bg-white h-full overflow-y-auto">
            ...
            @auth
                <a href="{{ route('profile.show') }}" class="py-2 px-4 text-sm flex items-center text-trueGray-500  hover:bg-orange-500 hover:text-white">
                    <span class="d-flex justify-center w-9"> <i class="far fa-address-card"></i></span>
                    Perfil
                </a>
                <a href=""
                    ...
                    <span class="d-flex justify-center w-9"> <i class="fas fa-sign-out-alt"></i></span>
                    Cerrar Sesión
                </a>
            ...
            @else
                <a href="{{ route('login') }}" class="py-2 px-4 text-sm flex items-center text-trueGray-500  hover:bg-orange-500 hover:text-white">
                    <span class="d-flex justify-center w-9"> <i class="fas fa-user-circle"></i></span>
                    Iniciar Sesión
                </a>
                <a href="{{ route('register') }}" class="py-2 px-4 text-sm flex items-center text-trueGray-500  hover:bg-orange-500 hover:text-white">
                    <span class="d-flex justify-center w-9"> <i class="fas fa-fingerprint"></i></span>
                    Registrate
                </a>
            @endauth
        </div>
